...
try:
    with engine.connect() as connection:
        ...
        for row in result:
            print(str(row))
        result = connection.execute("DROP TABLE IF EXISTS mViz_Arabidopsis_" + chromosome + "_Motif_Sequence CASCADE;")
except Exception as e:
    print("It is not working.")
    sys.exit(1)

folder_path = Path("/data/yenc/projects/2022_03_24_mViz/2022_05_01_upload_processed_arabidopsis_TF/output")
file_path = folder_path.joinpath('Arabidopsis_thaliana_binding_TFBS_from_motif_genome-wide_Ath_'+str(chromosome)+'.txt')
...
dat.to_sql(
    name="mViz_Arabidopsis_" + chromosome + "_Motif_Sequence",
    chunksize=1000,
    con=engine,
    if_exists='append',
    index=False,
...
# Index table
#######################################################################
output_table = "mViz_Arabidopsis_" + chromosome + "_Motif_Sequence"
try:
    with engine.connect() as connection:
