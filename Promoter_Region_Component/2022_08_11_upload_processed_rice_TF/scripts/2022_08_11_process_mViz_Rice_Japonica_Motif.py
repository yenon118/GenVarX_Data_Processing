...
def process_line(line_array, reference_dict, output_array):
    ...
    gene_id = str(line_array[2]).strip()

    if motif.startswith('LOC_') and gene_id.startswith('LOC_'):
        if motif.startswith('LOC_'):
            motif = re.sub("^LOC_", "", motif)
        if gene_id.startswith('LOC_'):
            gene_id = re.sub("^LOC_", "", gene_id)
        
        if motif in reference_dict.keys() and gene_id in reference_dict.keys():
            output_array.append(
                reference_dict[motif] + "\t" + 
