...
try:
    with engine.connect() as connection:
        ...
        for row in result:
            print(str(row))
        result = connection.execute("DROP TABLE IF EXISTS mViz_Rice_Japonica_Motif_Sequence CASCADE;")
except Exception as e:
    print("It is not working.")
    sys.exit(1)

folder_path = Path("/data/yenc/projects/2022_08_11_mViz_KBC/2022_08_11_upload_processed_rice_TF/output")
file_path = folder_path.joinpath('Oryza_sativa_Japonica_Group_TFBS_from_motif_genome-wide_Osj.gff')
...
elif str(file_path).endswith('gff'):
    dat = pd.read_table(
        filepath_or_buffer=file_path,
        sep="\t"
    )
...
dat.to_sql(
    name='mViz_Rice_Japonica_Motif_Sequence',
    chunksize=1000,
    con=engine,
    if_exists='append',
    index=False,
...
# Index table
#######################################################################
output_table = "mViz_Rice_Japonica_Motif_Sequence"
try:
    with engine.connect() as connection:
