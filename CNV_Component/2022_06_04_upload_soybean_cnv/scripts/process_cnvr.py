...
def process_line(header_array, line_array, output_array):
    chromosome = str(line_array[1]).strip()
    start = str(line_array[2]).strip()
    end = str(line_array[3]).strip()
    width = str(line_array[4]).strip()
    strand = str(line_array[5]).strip()

    for i in range(6, len(header_array)):
        ...
        if (accession.startswith("USB.")):
            accession = re.sub("(USB\\.)", "USB-", accession)
        if (accession.startswith("SJ.")):
            accession = re.sub("(SJ\\.)", "SJ-", accession)
