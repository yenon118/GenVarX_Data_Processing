...
try:
    with engine.connect() as connection:
        ...
        for row in result:
            print(str(row))
        result = connection.execute("DROP TABLE IF EXISTS mViz_Rice_Nipponbare_CNVR CASCADE;")
except Exception as e:
    print("It is not working.")
    sys.exit(1)

folder_path = Path("/data/yenc/projects/2022_08_11_mViz_KBC/2022_08_11_upload_rice_cnv/output/Nipponbare")
file_path = folder_path.joinpath('cnvr.txt')
...
dat.to_sql(
    name='mViz_Rice_Nipponbare_CNVR',
    chunksize=1000,
    con=engine,
    if_exists='append',
    index=False,
...
try:
    with engine.connect() as connection:
        sql_str = "CREATE INDEX mViz_Rice_Nipponbare_CNVR_accession_idx ON mViz_Rice_Nipponbare_CNVR(Accession);"
        result = connection.execute(sql_str)
        # print(sql_str)
        sql_str = "CREATE INDEX mViz_Rice_Nipponbare_CNVR_start_idx ON mViz_Rice_Nipponbare_CNVR(Start);"
        result = connection.execute(sql_str)
        # print(sql_str)
        sql_str = "CREATE INDEX mViz_Rice_Nipponbare_CNVR_end_idx ON mViz_Rice_Nipponbare_CNVR(End);"
        result = connection.execute(sql_str)
        # print(sql_str)
        sql_str = "CREATE INDEX mViz_Rice_Nipponbare_CNVR_chromosome_idx ON mViz_Rice_Nipponbare_CNVR(Chromosome);"
        result = connection.execute(sql_str)
        # print(sql_str)
except Exception as e:
    print("It is not working.")
